...
function dateToTimestamp(date) {
  const newDate = new Date('01 Jan 1970 00:00:00 UTC');
  return new Date(date).getTime() - newDate.getTime();
}
...
function getNextFriday(date) {
  ...
  const nextWeek =
    date.getUTCDay() < 5 ? 5 - date.getUTCDay() : 12 - date.getUTCDay();
  newDate.setDate(date.getDate() + nextWeek);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const result = new Date(endDate.getTime() - startDate.getTime());
  return result / (24 * 60 * 60 * 1000) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  if (myDate >= startDate && myDate <= endDate) return true;
  return false;
}
...
function getWeekNumberByDate(date) {
  ...
  const diff = (date.getTime() - firstDay.getTime()) / (24 * 60 * 60 * 1000);
  const result = diff + firstDay.getUTCDay() + 1;
  return Math.ceil(result / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && startDate <= endDate; i += 1) {
      workSchedule.push(formattingDateForWorkSchedule(currentDate));
      currentDate.setDate(currentDate.getDate() + 1);
    }

    for (let i = 0; i < countOffDays && startDate <= endDate; i += 1) {
